...
thread_init (void)
{
  ...
  // thread is unblocked(not sleeping) when initializing
...
thread_tick (int64_t tick)
{
  ...
  // wake up sleeping threads using current tick
  thread_awake(tick);
...
/* Make the current thread t sleep until the timer ticks
   reaches 'ticks_end'.

   It will block current thread so that it won't consume CPU resourses, unless it's unblock after ending ticks.
